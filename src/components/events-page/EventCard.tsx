    ...
    return (
        <motion.div
            ...
            <Image src={props.image} alt={"batch-image"} width={300} height={300} draggable={false}
                className={`object-cover relative z-10 max-sm:w-[15rem] ${props.inverted ? "md:right-44" : "md:left-44"}`} />
            <div className="md:w-full bg-black/40 hover:bg-black/50 transition-all duration-150 
            hover:backdrop-blur-sm  md:h-[16rem] rounded-md p-5 md:p-7 md:px-36 border border-transparent hover:border-y-white/30">
                <p className="text-2xl sm:text-4xl text-center md:text-start font-semibold  ps-4 capitalize text-yellow-100">{props.category} :</p>
                <>
                    {props.events.map((event, index) => (
                        <div key={index} className="flex my-4 md:mx-56 w-full md:w-2/3 justify-between items-center">
                            <p className="md:mr-20 text-lg md:text-2xl capitalize">{event.name}</p>
                        ...
                        tracking-tighter text-white border border-white/75 rounded-lg group cursor-pointer">
                                    ...
                                    <span className="relative sm:group-hover:text-black transition-all duration-75">Learn more</span>
                                </p>
                            </Link>
                        </div>
                    ))}
