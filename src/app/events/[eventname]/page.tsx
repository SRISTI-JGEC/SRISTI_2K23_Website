...
const Page = () => {
  const [ParicularEvent, SetParticularEvent] = useState<EventData>();
  const router = useRouter();
  const { eventname } = useParams();

  const FindEvent = () => {
    const data = EventList.find((items) => items.category === eventname);
    console.log(data);
    SetParticularEvent(data!);
  };
  useEffect(() => {
    FindEvent();
  }, []);
      ...
      <div className="min-h-screen">
        ...
        <div className="event-timing min-h-[60vh] flex flex-col items-center justify-start w-full text-white p-10 bg-center bg-cover bg-no-repeat">
          ...
          <div className="actions flex justify-between w-[30rem] max-[640px]:w-[20rem] max-[280px]:w-[15rem]">
            ...
            {/* <Link href={ParicularEvent.leaderboard}>
