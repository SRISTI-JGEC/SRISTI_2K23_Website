  ...
  try {
    ...
    if (team && (team1 || team2)) {
      return NextResponse.json(
        {
          error:
            "Team already exists or Lead is already been registered in an other teeam",
        },
        { status: 400 }
      );
    }
    ...
    for (let i = 0; i < members.length; i++) {
      let find = await Team.findOne({
        eventName,
        "members.email": members[i].email,
      });
      let find2 = await Team.findOne({
        eventName,
        leadEmail: members[i].email,
      });
      if (find || find2)
        return NextResponse.json(
